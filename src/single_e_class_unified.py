...
class single_electron:
    def __init__(self,file_name=None, dim_parameter_dictionary={}, simulation_options={}, other_values={}, param_bounds={}, results_flag=True):
        ...
        self.other_values["harmonic_range"]=self.other_values["harmonic_range"]
        self.num_harmonics=len(self.other_values["harmonic_range"])
        ...
        last_point= (self.other_values["harmonic_range"][-1]*self.nd_param.nd_param_dict["omega"])+(self.nd_param.nd_param_dict["omega"]*self.filter_val)
    ...
    def top_hat_filter(self, time_series):
        ...
        if "fourier_scaling" in self.simulation_options:
            if self.simulation_options["fourier_scaling"]!=None:
                scale_flag=True
        if sum(np.diff(self.other_values["harmonic_range"]))!=len(self.other_values["harmonic_range"])-1 or scale_flag==True or self.other_values["filter_val"]!=0.5:
            results=np.zeros(len(top_hat), dtype=complex)
            for i in range(0, self.num_harmonics):
                true_harm_n=true_harm*self.other_values["harmonic_range"][i]
            ...
            first_harm=(self.other_values["harmonic_range"][0]*true_harm)-(true_harm*self.filter_val)
            last_harm=(self.other_values["harmonic_range"][-1]*true_harm)+(true_harm*self.filter_val)
